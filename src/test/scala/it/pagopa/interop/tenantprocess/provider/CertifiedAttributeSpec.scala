...
class CertifiedAttributeSpec extends AnyWordSpecLike with SpecHelper with ScalatestRouteTest {
  ...
  "Certified attribute addition" should {
    ...
    "succeed if certified tenant attribute already exists" in {
      implicit val context: Seq[(String, String)] = adminContext
      ...
      val attribute =
        Dependency.CertifiedTenantAttribute(id = seed.id, assignmentTimestamp = timestamp, revocationTimestamp = None)

      val managementSeed = Dependency.TenantAttribute(declared = None, certified = Some(attribute), verified = None)

      mockDateTimeGet()
      mockGetTenantById(organizationId, requester)
      mockGetTenantById(tenantId, tenant)
      mockGetAttributeById(seed.id, persistentAttribute.copy(id = seed.id, origin = Some("IPA")))
      mockUpdateTenantAttribute(tenantId, seed.id, managementSeed, dependencyTenant)
      mockUpdateTenant(
        tenantId,
        Dependency.TenantDelta(
          selfcareId = None,
