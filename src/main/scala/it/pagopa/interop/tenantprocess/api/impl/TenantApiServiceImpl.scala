...
final case class TenantApiServiceImpl(
  ...
  override def addCertifiedAttribute(tenantId: String, seed: CertifiedTenantAttributeSeed)(implicit
    ...
    val result: Future[Tenant] = for {
      ...
      )(attr => attr.revocationTimestamp match {
        case None => Future.failed(CertifiedAttributeNotRevoked(targetTenantUuid, attribute.id))
        case Some(_) => tenantManagementService.updateTenantAttribute(targetTenantUuid, attribute.id, attribute.toCertifiedSeed(now))
        }
      )
